...
foreach ($pratiche as $pratica) {
	...
	exec("sshpass -p \"Kine993\" scp -r 'mpaoli@crono.kine.coop:/home/groups/archiviDigitali/37-TREVILLE/01.CONSEGNE/".$pratica->attribute("name")."' '".$storageDir.$pratica->attribute("name")."'");
		...
		if (is_dir($storageDir.$pratica->attribute("name"))) {
				...
				$childs = $pratica->subTree();
				$nodi = array();				
				foreach ($childs as $figlio) {
					$nodi[] = $figlio->attribute("node_id");
				}
				if(sizeof($nodi)>0) {
					eZContentObjectTreeNode::removeSubtrees	($nodi,false);	
				}
