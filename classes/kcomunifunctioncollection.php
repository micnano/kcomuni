...
class kcomuniFunctionCollection
{
    ...
    public static function fetchArticleHeader($catID)
    {
       if( is_numeric( $catID ) )
		...
		public static function checkpart($nome,$ContainerID,$comune,$tipo){
		
			$params['ClassFilterType'] = "include";
 			$params['SortBy'] = array('published', false);
 			$params['ClassFilterArray'] = array('area');
			$params['AttributeFilter'] = array( 
				array('area/tipo', '=', $tipo ),
				array('area/numero', '=', $nome ),
				array('area/cc', '=', $comune ) );
			$Node = eZContentObjectTreeNode::fetch($ContainerID);
			$actors = $Node->subTree($params);
			if(sizeof($actors)>0) {
				...
				$params= array();
				$params['class_identifier'] = "area";
				$params['section_id'] = 1;
				$params['creator_id'] = 14;
				$params['parent_node_id'] = $ContainerID;
				$attributesData = array() ;
				$attributesData['tipo'] =  $tipo;
				$attributesData['numero'] =  $nome;
				$attributesData['cc'] =  $comune;
				$params['attributes'] = $attributesData;
				$contentObject = eZContentFunctions::createAndPublishObject($params);										
				return $contentObject->ID;
			}
 ...
 public function getComuniSearchFacets()
 {
 	...
 	$facets[] = array( 'field' => 'pratica/cc',
 			'name'  => "Atto: Comune Catastale",
 			'limit' => $limit );
